...
#include <math.h>
	...
	public:
		...
		inline Vector3DF operator+ (Vector3DF op)	{ return Vector3DF((float) x+op.x, (float) y+op.y, (float) z+op.z); }
		inline Vector3DF operator- (Vector3DF op)	{ return Vector3DF((float) x-op.x, (float) y-op.y, (float) z-op.z); }
...
//		inline Vector4DF (Vector4DF op)		{ x=op.x; y=op.y; z=op.z; w=op.w; }
		...
		inline Vector4DF operator+ (int op)			{ return Vector4DF(x+float(op), y+float(op), z+float(op), w); }
		inline Vector4DF operator+ (float op)		{ return Vector4DF(x+op, y+op, z+op, w); }
		inline Vector4DF operator+ (Vector4DF op)	{ return Vector4DF(x+op.x, y+op.y, z+op.z, w); }
		inline Vector4DF operator- (int op)			{ return Vector4DF(x-float(op), y-float(op), z-float(op), w); }
		inline Vector4DF operator- (float op)		{ return Vector4DF((float) x-op, (float) y-op, (float) z-op, w); }
		inline Vector4DF operator- (Vector4DF op)	{ return Vector4DF((float) x-op.x, (float) y-op.y, (float) z-op.z, w); }
		inline Vector4DF operator* (int op)			{ return Vector4DF(x*float(op), y*float(op), z*float(op), w); }
		inline Vector4DF operator* (float op)		{ return Vector4DF((float) x*op, (float) y*op, (float) z*op, w); }
		inline Vector4DF operator* (Vector4DF op)	{ return Vector4DF((float) x*op.x, (float) y*op.y, (float) z*op.z, w); }		
		// --
