...
int Image::read (const char* filename, const char* alphaname )
{
	...
	if ( file_a != 0x0 ) fseek(file_a, 0, SEEK_SET);
...
int Image::readBMP (FILE* fp, FILE* fp_a, bool bBaseImg  )
{
	...
	if ( bBaseImg ) {	
		...
		case 1:		channels = 1;	break;		// 1 byte per bit - *bad storage, 2 color (B & W)
		case 4:		channels = 1;	break;		// 1 byte per 4-bits - not great, 16 color, index mode
		case 8:		channels = 1;	break;		// 1 byte per pixel, 256 color, index mode
		case 16:	channels = 3;	break;		// 2 byte per pixel, 16-bit color
		case 24:	channels = 3;	break;		// 3 byte per pixel (RGB), 24-bit color 
		case 32:	channels = 3;	break;		// 4 byte per pixel, 32-bit color, compressed down to 24-bit
		}
			...
			for (int y = 0; y < height; ++y) {
				...
				for (int x = 0; x < width; ++x) {
					...
					if ( bBaseImg ) {
						...
						setAlpha(x, height - 1 - y, palette[index].rgbRed / 255.0f );
					}
				}
			}
			delete [] scanlineByte;
			...
			for (int y = 0; y < height; ++y) {
				...
				for (int x = 0; x < width; ++x) {
					...
					if ( bBaseImg ) {						
						...
						setAlpha(x, height - 1 - y, (pixel.r+pixel.g+pixel.b)/3.0 );
					}
				}
			}
			delete [] scanline32;
